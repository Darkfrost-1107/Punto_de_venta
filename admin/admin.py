...
class VistaProductos(Screen):
	...
	def confirmar_eliminacion(self, confirmado):
		if confirmado:
			indice = self.ids.rv_productos.dato_seleccionado()
			if indice >= 0:
				producto_tuple = (self.ids.rv_productos.data[indice]['codigo'],)
	...
	def eliminar_producto(self):
		...
		if indice >= 0:
			...
			# Crear y abrir el popup de confirmación
			popup = ConfirmacionEliminarPopup(self.confirmar_eliminacion)
			popup.abrir(producto)
...
class ConfirmacionEliminarPopup(Popup):
	...
	def abrir(self, producto):
		self.ids.confirmacion_texto.text = f"¿Está seguro de que desea eliminar el producto\n'{producto['nombre']}' (Código: {producto['codigo']})?"
		self.open()

	def confirmar(self):
		self.confirmar_callback(True)
		self.dismiss()
...
class VistaVentas(Screen):
	...
	def crear_excel(self):
		...
		if self.ids.ventas_rv.data:
			...
			# Define the folder for saving Excel files
			excel_nombre = path.__str__() + '\\ventas_excel\\'
			isExist = os.path.exists(excel_nombre)
			if not isExist:
				os.makedirs(excel_nombre)

			# Define the Excel file name
			excel_nombre += self.ids.date_id.text + '.xlsx'
			...
			for venta in self.productos_actuales:
				for item in venta:
					item_found = next((producto for producto in productos_excel if producto["codigo"] == item[3]), None)
					total += item[2] * item[4]
					if item_found:
			...
			# Define the headers
			headers = ['Nombre', 'Código', 'Cantidad', 'Precio', 'Precio Total']
			ws.append(headers)
			...
			# Save the Excel file
			wb.save(excel_nombre)
			self.ids.notificacion.text = 'Excel creado y guardado'
		else:
