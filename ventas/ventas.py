...
class PagarPopup(Popup):
	...
	def mostrar_cambio(self):
		self.recibido = self.ids.recibido.text
		try:
...
class CustomDropDown(DropDown):
	def __init__(self, main_widget, **kwargs):
		...
		connection = QueriesSQLite.create_connection("pdvDB.sqlite")
		inventario_sql = QueriesSQLite.execute_read_query(connection, "SELECT * from productos")
		inventario = [producto_sql[1] for producto_sql in inventario_sql]
		self.main_widget = main_widget
		self.all_items = inventario
	...
	def select_item(self, text):
		self.main_widget.text = text
		self.main_widget.focus = True  # Mantiene el foco en el TextInput
		self.dismiss()
...
class VentasWindow(BoxLayout):
	...
	def __init__(self, actualizar_productos_callback, **kwargs):
		...
		self.ids.buscar_nombre.bind(text=self.on_text_change)
		self.dropdown = CustomDropDown(self.ids.buscar_nombre)

		self.generador = GeneradorRecibos()

	def on_text_change(self, instance, value):
		if value:
			self.dropdown.update_dropdown(value)
	...
	def pagar(self):
		if self.ids.rvs.data:
			...
			# Resaltar el botón de Nueva Compra (cambia el color de fondo y texto)
			self.ids.nueva_compra.background_color = (0.3, 0.6, 1, 1)  # Color azul para resaltado
			self.ids.nueva_compra.color = (1, 1, 1, 1)  # Texto blanco
		else:
	...
	def nueva_compra(self, desde_popup=False):
		if desde_popup:
			...
			# Restaurar el botón de Nueva Compra a su estado original
			self.ids.nueva_compra.background_color = (0.22, 0.22, 0.22, 1)
