...
Builder.load_file('compras/compras.kv')
...
class ComprasWindow(BoxLayout):
	...
	def pagado(self):
		...
		venta = """ INSERT INTO compras (total, fecha, username) VALUES (?, ?, ?) """
		venta_tuple = (self.total, self.ahora, self.usuario['username'])
		venta_id = QueriesSQLite.execute_query(connection, venta, venta_tuple)
		compras_detalle = """ INSERT INTO compras_detalle(id_venta, precio, producto, cantidad) VALUES (?, ?, ?, ?) """
		...
		for producto in self.ids.rvs.data:
			...
			compras_detalle_tuple= (venta_id, producto['precio'], producto['codigo'], producto['cantidad_carrito'])
			actualizar_admin.append({'codigo': producto['codigo'], 'cantidad': nueva_cantidad})

			QueriesSQLite.execute_query(connection, compras_detalle, compras_detalle_tuple)
